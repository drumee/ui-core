...
const { permission } = require("./constants/acl");
//########################################
 ...
 */
   ...
   * Example :
   *  Visitor.domainCan(permission.admin_member)  
   *  Visitor.domainCan(permission.admin_member, user.privilege)  
   * 
   * @param {*} permission 
   * @param {*} userPrivilege 
   * @returns 
   */
  domainCan(p, userPrivilege) {
    if (userPrivilege == null) {
      userPrivilege = this.get(_a.privilege);
    }
    return userPrivilege & p;
  }
   ...
   */
  canServerImpExp() {
    return this.domainCan(permission.admin_security);
  }
