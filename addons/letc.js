 ...
 * 
 */
p.mergeKidsOptions = function (kids) {
 ...
 * @returns 
 */
p.registerPart = function (child, name) {
 ...
 * @param {*} e 
 */
p.__handleContextmenu = function (e) {
  e.stopPropagation();
  e.stopImmediatePropagation();
  if (e.shiftKey || e.ctrlKey) {
 ...
 * @param {*} e 
 */
p.__handleMouseenter = function (e) {
  if (this.__tooltips) {
 ...
 * @param {*} e 
 */
p.__handleMouseleave = function (e) {
  if (this.__tooltips) {
    let tt = this.__tooltips;
    this.__tooltips.state = 1;
    $(tt.el).hide();
...
p._responsive = function (refresh = 1) {
 ...
 * @returns 
 */
p.declareHandlers = function (opt = {}) {
  const defaults = {
    part: _a.single,
    ui: _a.single
  };
 ...
 * @returns 
 */
p.actualNode = function (format = _a.orig) {
  let m = this.model;
  let nid = m.get(_a.nodeId);
  let home_id = m.get(_a.home_id);
  if ([null, undefined, '0', 0, '*'].includes(nid)) {
    nid = home_id;
  }
  let hub_id = m.get(_a.hub_id);
  let vhost = m.get(_a.vhost);
  if (m.get(_a.isalink) && this.metadata) {
  ...
  let href;
  let filepath = m.get(_a.ownpath) || m.get(_a.filepath) || this.media?.mget(_a.filepath);
  let filetype = m.get(_a.filetype) || this.media?.mget(_a.filetype);

  const md5Hash = m.get('md5Hash');
  const changed = Math.abs(m.get(_a.mtime) - m.get(_a.ctime));
  let killCache;
  ...
  if (keysel && m.get(_a.area) != _a.public) {
  ...
  let filename = m.get(_a.filename);
  if (m.get(_a.ext)) {
    filename = `${filename}.${m.get(_a.ext)}`
  }
 ...
 * @param {*} args 
 */
p.waitElement = function (el, handler, args) {
